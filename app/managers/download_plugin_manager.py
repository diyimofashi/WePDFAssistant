...
class DownloadPluginManager:
    """下载插件管理器"""
    
    def __init__(self):
        self.plugins: Dict[str, DownloadPluginInterface] = {}
            ...
            if os.path.isdir(plugin_dir):
                ...
                if os.path.exists(plugin_module_file):
                    try:
                        ...
                        # 查找插件类（通常以Download开头的类）
                        plugin_class = None
                        for attr_name in dir(plugin_module):
                            attr = getattr(plugin_module, attr_name)
                            if (isinstance(attr, type) and 
                                issubclass(attr, DownloadPluginInterface) and 
                                attr != DownloadPluginInterface):
                                plugin_class = attr
                                break
                        ...
                        if plugin_class:
                            ...
                            logger.info(f"成功加载下载插件: {plugin_dir_name}")
                        else:
                            logger.warning(f"在 {plugin_module_file} 中未找到下载插件类")
                            
                    except Exception as e:
                        logger.error(f"加载下载插件 {plugin_dir_name} 失败: {e}")
    ...
    def initialize_plugin(self, plugin_name: str, config: Dict[str, Any]) -> DownloadResult:
        """
        初始化指定插件
    ...
    def download_with_plugin(self, plugin_name: str, url: str, local_path: str, **kwargs) -> DownloadResult:
        """
        使用指定插件下载文件
    ...
    def download_bytes_with_plugin(self, plugin_name: str, url: str, **kwargs) -> DownloadResult:
        """
        使用指定插件下载文件为字节流
    ...
    def get_plugin(self, plugin_name: str) -> DownloadPluginInterface:
        """
        获取指定插件实例
...
# 全局下载插件管理器实例
download_plugin_manager = DownloadPluginManager()