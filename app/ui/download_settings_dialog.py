...
logger = get_logger('download_settings_dialog')
...
class DownloadSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # 初始化配置管理器
        self.plugin_manager = download_plugin_manager
        self.config_manager = download_config_manager
    ...
    def setup_ui(self):
        ...
        current_plugin_label = QLabel("当前使用的下载插件:")
    ...
    def create_plugin_settings_tabs(self):
        ...
        if not plugins:
            ...
            no_plugin_label = QLabel("暂无下载插件加载")
    ...
    def reset_settings(self):
        ...
        reply = QMessageBox.question(self, "确认重置", 
                                   f"确定要将 {current_plugin_name} 的下载设置重置为默认值吗？",
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.information(self, "重置完成", f"{current_plugin_name} 的下载设置已重置为默认值")
            except Exception as e:
                logger.error(f"重置配置时出错: {e}")
                QMessageBox.critical(self, "重置失败", f"重置配置时发生错误: {str(e)}")


def show_download_settings_dialog(parent=None):
    """显示下载设置对话框"""
