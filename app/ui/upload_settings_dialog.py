...
logger = get_logger('upload_settings_dialog')
...
class UploadSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # 初始化插件管理器和配置管理器
        self.plugin_manager = upload_plugin_manager
    ...
    def setup_ui(self):
        ...
        current_plugin_label = QLabel("当前使用的上传插件:")
    ...
    def create_plugin_settings_tabs(self):
        ...
        if not plugins:
            ...
            no_plugin_label = QLabel("暂无上传插件加载")
    ...
    def reset_settings(self):
        ...
        reply = QMessageBox.question(self, "确认重置", 
                                   f"确定要将 {current_plugin_name} 的上传设置重置为默认值吗？",
        ...
        if reply == QMessageBox.Yes:
            try:
                # 只重置当前插件的配置
                plugin = self.plugin_manager.get_plugin(current_plugin_name)
                if plugin and hasattr(plugin, 'PluginInfo'):
                    plugin_info = plugin.PluginInfo
                    default_config = {}
                    ...
                    # 设置默认配置
                    self.config_manager.set_plugin_config(current_plugin_name, default_config)
                    
                    # 重新加载当前tab的设置
                    self.load_plugin_settings(current_plugin_name)
                    
                    QMessageBox.information(self, "重置完成", f"{current_plugin_name} 的上传设置已重置为默认值")
            except Exception as e:
                logger.error(f"重置配置时出错: {e}")
                QMessageBox.critical(self, "重置失败", f"重置配置时发生错误: {str(e)}")


def show_upload_settings_dialog(parent=None):
    """显示上传设置对话框"""
    dialog = UploadSettingsDialog(parent)
    return dialog.exec_()